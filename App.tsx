...
import { AnalysisResult, AppMode, HistoryItem, RuntimeMode, UserSettings, DEFAULT_SETTINGS, ChatMessage } from './types';
...
import { getRuntimeMode, setRuntimeMode } from './services/providers';
...
const App: React.FC = () => {
  const SETTINGS_KEY = 'prompixSettings';
  const HISTORY_KEY = 'prompixHistory';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const legacySettings = !storedSettings ? await get('visionLearnSettings') : null;
        if (storedSettings) {
        ...
        const storedHistory = await get(HISTORY_KEY);
        const legacyHistory = !storedHistory ? await get('visionLearnHistory') : null;
        if (storedHistory) {
          setHistoryItems(storedHistory);
        } else if (legacyHistory) {
          setHistoryItems(legacyHistory);
          await set(HISTORY_KEY, legacyHistory);
        }
      ...
      <main>
        ...
        {mode === 'history' && (
          <History
            ...
            systemLanguage={settings.systemLanguage}
            promptOutputLanguage={settings.promptOutputLanguage}
          />
        )}
      </main>
    </div>
