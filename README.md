...
### 🔒 3. 非破坏性维度卡片微调 (Non-Destructive Chat Card Locking)
...
### 📦 4. 存储分层与大图解耦策略 (Dual-Layer Decoupled Storage Architecture)
*   **技术实现**：
...
### 🚀 5. 服务端多 Provider 动态路由网关 (Stateless Gateway & SSE Streams)
...
### 🔍 6. 术语挖掘与 Wordbank 沉淀 (Visual Term Printer & glossary Mining)
...
### 🌾 7. 莫兰迪自然呼吸感设计 (Poetic Morandi Theme & Smooth Physics)
...
### 🧠 8. 双模型降本路由架构 (Dual-Model Cost-Reduction Routing)
*   **技术实现**：引入“多模态视觉分析大模型 + 轻量纯文本辅助模型”的双模型路由机制。对于大图分析、带图追问等视觉任务路由至多模态大模型；对于纯文本翻译、长尾术语解释、无图文字追问等，在不降级交互效果的前提下，智能动态路由至极低成本的轻量辅助模型。
*   **💡 产品决策思维 (PM Rationale)**：
    *   多模态大模型的 Token 价格通常是纯文本模型的数倍。若全流程（包括简单的术语解释和纯文字追问）都调用多模态模型，API 账单会成倍飙升。
    *   通过这一路由架构，我们能为运营方 and 个人用户**削减 60% 以上的 API 调用成本**，兼顾高保真体验与高性价比。

### 🔒 9. 视觉词汇库上下文隔离 (Visual Wordbank Context Isolation)
*   **技术实现**：实现了以单张图片为核心工作区的视觉词汇隔离机制。每次用户上传并分析一张新图，或在历史库中激活不同作品时，词汇库会自动清理并重新挖掘，仅呈现**当前激活图片专属**的视觉词汇与深度释义，彻底消除历史遗留词条的混淆。
*   **💡 产品决策思维 (PM Rationale)**：
    *   传统的词汇库往往是全局累加展示，导致用户在处理不同风格的作品时，无法分清哪些词来自当前任务，哪些是上一张图的遗留物。
    *   **上下文绝对隔离**能够显著降低用户的“信息过载”和“认知混淆”，让创作者专注于当下这幅画面的词汇打磨与追问。
...
│   └── tests/                 # 测试矩阵
│       ├── unit/              # 76 个 Vitest 单元测试 (包含预取队列、词汇隔离、自适应分类纠偏、降本旁路、降级路由等 22 个测试文件)
...
### 🧪 1. 76 个 Vitest 单元测试 (22 个测试文件)
覆盖核心路由、降级机制、缓存、预取队列、词汇隔离与分类自适应纠偏等模块：
*   **存储稳定性**：验证了 Base64 大图隔离写入 IndexedDB 且 Metadata 正常落库的可用性。
*   **语言降级与翻译**：确保模型即使返回残缺的 JSON 也能通过 `model-output.ts` 完成高容错净化归一化。
*   **分类自适应纠偏与隔离**：验证画幅、分辨率等字段的自动归类，并确保词汇库上下文隔离，干净启动。
*   **预取排队与旁路**：测试按优先级加载词条释义、最多预取 15 个，并验证本地静态释义 0 Token 拦截旁路的可用性。
...
作为一个具备敏捷规划能力的 AI 产品经理，我为 Prompix 规划了清晰的迭代里程碑，目前项目已升级至 **v1.3.0 (最新迭代发布)**。
...
- [x] **v1.2.0 - 语义去污、智能降级与动效美化 (Performance & Aesthetic)**: 
  - 优化 Wordbank 提取逻辑，排除段落级长句，防止接口卡死。
  - 移除导致 TLS 超时的 Google Fonts 握手，使本地编译提升 15s。
  - 重构 UI，全面采用莫兰迪微动效与 Particle 浮动背景。
  - 实现智能 Fallback 路由映射，解决用户错误配置文本模型时多模态图片的报错体验。
- [x] **v1.3.0 - 双模型路由与词汇库隔离 (Dual-Model Routing & Context Isolation)**:
  - 实现双模型架构（多模态视觉模型 + 轻量纯文本模型），纯文字交互智能路由降本，削减 60%+ 运行开销。
...
- [ ] **v1.4.0 - 下一代规划：跨模态语义化搜索 (Semantic Search & Vector DB)**:
...
在 `next-web/.env.local` 文件中填入基础配置（本地开发使用内置 Demo 提供商，或填入 Key 访问真实 API）：
```bash
NEXT_PUBLIC_RUNTIME_POLICY=local

# Managed Gemini 官方 API (平台直连必填，或在设置页自填 Key)
