...
interface Props {
    ...
    promptOutputLanguage?: 'zh' | 'en';
}

const History: React.FC<Props> = ({ items = [], onSelect, onDeleteItems, onMarkAsExported, systemLanguage, promptOutputLanguage = 'zh' }) => {
    ...
    const handleExport = () => {
        ...
        // 使用系统语言作为兜底，优先读取当前分析结果的正面语言约定
        const userFrontLanguage = promptOutputLanguage === 'zh' ? 'Chinese' : 'English';
      ...
      `;
        ...
        link.href = url;
        link.download = `prompix_export_${Date.now()}.xls`;
