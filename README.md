...
```mermaid
graph TD
    subgraph 客户端 (Client-Side SPA)
        U[用户上传图片 User Image] -->|Canvas 采样压缩| C[5KB 缩略图 + 1024px 主图]
        C -->|二进制大对象物理隔离| DB[(IndexedDB: 存储大图)]
        C -->|轻量化列表索引| LS[(LocalStorage: 历史列表)]
        C -->|请求载荷 Payload| GW[API Gateway]
    end
    
    subgraph 服务端 (API Gateway & Multi-Provider Router)
        GW -->|智能降级与重定向| RF{检测是否为纯文本模型?}
        RF -->|是: 自动路由至视觉模型| VP[Vision-Capable LLM Provider]
        RF -->|否: 保持原配置| VP
        VP -->|Zod Schema 强制规范| LLM[LLM API: OpenAI / SiliconFlow / Gemini / MiniMax]
        LLM -->|Bilingual Structured SSE 流式数据| GW
    end

    subgraph 交互与消费层 (Client Interaction)
        GW -->|0ms 语种瞬切| View[双语工作台 UI]
        View -->|聊天微调 Chat| Chat[非破坏性修改规约]
        Chat -->|无关卡片 100% 字符级锁定| View
        View -->|多格式一键导出| Export[Midjourney / SD / DALL-E / RAW]
        View -->|术语提取 Wordbank| Bank[术语词库挖掘与异步释义]
    end
```
